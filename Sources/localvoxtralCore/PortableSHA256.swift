...
/// SHA-256 (FIPS 180-4) in plain Swift, for platforms without CryptoKit.
/// `AppConfigStore.sha256Hex` uses CryptoKit on Apple platforms and this on
/// Linux, where the core's tests run; `PortableSHA256Tests` checks it against
/// the FIPS examples and, on a Mac, against CryptoKit.
package enum PortableSHA256 {
    ...
    package static func hex(of data: Data) -> String {
        digest(of: data).map { String(format: "%02x", $0) }.joined()
    }

    package static func digest(of data: Data) -> [UInt8] {
        // Padding: a 1 bit, zeros up to 56 bytes mod 64, then the message
