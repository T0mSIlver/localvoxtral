...
package enum CmuxPasswordValidation {
    /// UTF-8 byte cap. Generously above anything a human types; a value near it
    /// is not a password.
    package static let maxPasswordBytes = 256
...
/// above them.
package protocol CmuxKeychainBackend: Sendable {
...
public struct CmuxSocketPasswordStore: CmuxPasswordStoring {
    package static let defaultService = "com.localvoxtral.app.cmux-socket"
    package static let defaultAccount = "automation-socket-password"
...
package struct SecItemCmuxKeychainBackend: CmuxKeychainBackend {
    ...
    package func read(service: String, account: String) -> Data? {
    ...
    package func write(_ data: Data, service: String, account: String) -> Bool {
    ...
    package func delete(service: String, account: String) -> Bool {
        guard isKeychainPermitted else { return false }
        #if canImport(Security)
