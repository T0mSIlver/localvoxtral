...
package struct SocketPaneScreenCapture: Sendable, Equatable {
    ...
    package let paneKey: String

    package init(text: String, paneKey: String) {
        self.text = text
...
@MainActor
package enum SocketPaneScreenContext {
    ...
    package static func captureAtStart(
    ...
    package static func reconcileAtStop(
    ...
    private static func paneText(
        ...
        case .ttyDevice, .browserTab, .desktopSession, .remoteSSHConnection, .remoteLocalTTY:
            // A browser tab or a Claude Desktop window has no pane socket to
            // read, and no verified screen route of any kind — their joins buy
            // session/repository context only.
