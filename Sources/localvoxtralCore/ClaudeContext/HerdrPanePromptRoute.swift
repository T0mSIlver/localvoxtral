...
/// into an agent only through three routes") before widening it.
package struct HerdrPanePromptRoute: AgentPromptRoute {
    ...
    package func deliver(_ call: AgentPromptCall) async -> Bool {
        ...
        case .append(let text):
            guard Self.isSendable(text) else {
                Log.backends.notice("herdr pane route: text holds a control character or is too long; keystrokes instead")
                return false
            }
            return await writer.sendText(socketPath: binding.socketPath, paneID: binding.paneID, text: text)
        case .submit:
            ...
            return await writer.pressEnter(socketPath: binding.socketPath, paneID: binding.paneID)
        }
    }
    ...
    /// a key sequence. Such text goes by keystrokes, which type it as text.
