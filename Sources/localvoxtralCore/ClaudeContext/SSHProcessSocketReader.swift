...
package enum SSHConnectionAddressMatch {
    ...
    package static func addressBytes(_ text: String) -> [UInt8]? {
        guard !text.isEmpty,
    ...
    package static func unwrappingIPv4Mapped(_ bytes: [UInt8]) -> [UInt8] {
    ...
    package static func sameAddress(_ lhs: String, _ rhs: String) -> Bool {
...
/// not one.
package enum SSHProcessSocketReader {
    ...
    /// a syscall storm.
    package static let maxDescriptors = 512
    ...
    package static let tcpStateEstablished = Int32(TSI_S_ESTABLISHED)

    package static let live: @Sendable (Int32) -> [SSHClientSocket]? = { pid in
        establishedTCPSockets(pid: pid)
    }

    package static func establishedTCPSockets(pid: Int32) -> [SSHClientSocket]? {
...
package enum SSHProcessSocketReader {
    package static let live: @Sendable (Int32) -> [SSHClientSocket]? = { _ in nil }
}
#endif
#endif
