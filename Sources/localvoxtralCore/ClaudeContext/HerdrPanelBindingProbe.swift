...
package enum HerdrPanelBindingAbstention: String, Sendable, Equatable {
...
package struct HerdrPanelBindingProbe {
    ...
    package typealias GridRead = @MainActor @Sendable (TerminalScreenTarget) -> String?
    package typealias Now = @MainActor @Sendable () -> Date
    package typealias SleepFor = @Sendable (TimeInterval) async -> Void
    package typealias RandomBits = @MainActor @Sendable () -> UInt64

    package static let tokenTTLMilliseconds = 8_000
    package static let settleDelay: TimeInterval = 0.12
    ...
    package static let settleBudget: TimeInterval = 1.0
    package static let maxGridReads = 9
    ...
    package static let minimumRenderedNonceDigits = 8
    ...
    /// own cause.
    package enum RenderedTokenMatch: Sendable, Equatable {
        case full
    ...
    package static func renderedMatch(grid: String, token: String) -> RenderedTokenMatch {
    ...
    package static func gridGeometry(_ grid: String) -> (rows: Int, columns: Int) {
        let lines = grid.split(separator: "\n", omittingEmptySubsequences: false)
        return (rows: lines.count, columns: lines.map(\.count).max() ?? 0)
    }

    package static func token(randomBits: UInt64) -> String {
    ...
    package static func noteRowNotRendered(grid: String) {
    ...
    package static func noteRowTruncated(retainedDigits: Int) {
        Log.claudeContext.info(
            """
    ...
    package static func noteAbstention(_ cause: HerdrPanelBindingAbstention) {
        Log.claudeContext.info(
...
package final class HerdrPanelMicIndicator: @unchecked Sendable, Equatable {
    package typealias SleepFor = @Sendable (TimeInterval) async -> Void

    package static let refreshInterval: TimeInterval = 4
    ...
    package static func == (lhs: HerdrPanelMicIndicator, rhs: HerdrPanelMicIndicator) -> Bool {
        lhs === rhs
    }
    ...
    @discardableResult
    package func refreshOnce() async -> Bool {
    ...
    package func stopAndWait() async {
