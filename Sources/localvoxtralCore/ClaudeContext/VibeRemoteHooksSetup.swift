...
extension ClaudeRemoteEnrollmentService {
    ...
    /// for tests.
    package struct VibeHostProbe: Equatable {
        package var vibeFound = false
        package var installedVersion: String?
    ...
    package static let vibeProbeFrameBegin = "LVX_VIBE_PROBE_BEGIN"
    package static let vibeProbeFrameEnd = "LVX_VIBE_PROBE_END"
    package static let vibeHooksMaxBytes = 256 * 1024
    package static let vibeRemoteDirectory = "$HOME/.vibe/localvoxtral/remote"

    /// `vibe` is routinely off a non-interactive ssh PATH (uv and pipx install
    /// into `~/.local/bin`).
    package static let vibePathPrefix = "PATH=\"$HOME/.local/bin:$HOME/bin:/opt/homebrew/bin:/usr/local/bin:$PATH\"; export PATH\n"

    package static var vibeProbeScript: String {
        """
        set -u
    ...
    /// ignored.
    package static func vibeProbe(inFramedOutput output: String) -> VibeHostProbe? {
    ...
    package static func heredocDelimiter(for content: String, seed: String) -> String {
