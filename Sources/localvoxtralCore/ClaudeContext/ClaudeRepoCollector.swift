...
package struct ClaudeRepoSnapshot: Sendable, Equatable {
    ...
    package var statusLines: [String]
    package var stagedDiff: String
    package var unstagedDiff: String
    ...
    package var secretPaths: [String] = []
    package var provenance: ClaudeRepoProvenance

    package static let empty = ClaudeRepoSnapshot(
        workspaceName: "",
        branch: nil,
        statusLines: [],
        stagedDiff: "",
    ...
    /// selector inherits.
    package var allFiles: [File] { activeFiles + trackedFiles }
}
...
package struct ClaudeRepoProvenance: Sendable, Equatable {
    package var trackedFileCount = 0
    package var activeFileCount = 0
    package var snippetFileCount = 0
    package var statusLineCount = 0
    package var stagedDiffCharacters = 0
    package var unstagedDiffCharacters = 0
    ...
    package var skippedBinary = 0
    package var skippedGenerated = 0
    package var skippedLogs = 0
    ...
    /// actually contain.
    package var skippedUncontained = 0
    ...
    package var withheldDiffFiles = 0
    ...
    package var truncatedFiles = 0
    package var deadlineExpired = false

    package static let empty = ClaudeRepoProvenance()
...
package protocol ClaudeRepoCollecting: Sendable {
    func collect(
...
package struct ClaudeRepoCollector: ClaudeRepoCollecting {
...
extension ClaudeRepoProvenance {
    ...
    package mutating func merge(_ other: ClaudeRepoProvenance) {
...
package enum LocalWorkspacePathNormalization {
    package static func normalize(_ path: String) -> String {
...
package struct ClaudeLocalFileSystem: ClaudeLocalFileReading {
    package func isDirectory(_ path: LocalWorkspacePath) -> Bool {
        metadata(path).map { ($0.st_mode & S_IFMT) == S_IFDIR } ?? false
    }

    package func isRegularFile(_ path: LocalWorkspacePath) -> Bool {
        metadata(path).map { ($0.st_mode & S_IFMT) == S_IFREG } ?? false
    }

    package func fileSize(_ path: LocalWorkspacePath) -> Int? {
        metadata(path).map { Int($0.st_size) }
    }

    package func readFile(_ path: LocalWorkspacePath, maxBytes: Int) -> Data? {
