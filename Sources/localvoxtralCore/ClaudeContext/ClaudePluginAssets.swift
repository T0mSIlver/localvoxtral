...
public enum ClaudePluginAssets {
    ...
    package static let assetsSourceFile = #filePath
    ...
    package static func developmentMarketplaceURL(
    ...
    /// cannot drift.
    package static func rootMarketplaceURL(sourceFile: String = ClaudePluginAssets.assetsSourceFile) -> URL? {
        repositoryRootURL(sourceFile: sourceFile)
    }

    package static func repositoryRootURL(sourceFile: String) -> URL? {
    ...
    package static func developmentOpencodePluginURL(
        sourceFile: String = ClaudePluginAssets.assetsSourceFile
    ) -> URL? {
        repositoryRootURL(sourceFile: sourceFile)?
    ...
    /// The bundled opencode plugin file: `bundleResourcesURL` first, the app
    /// bundle's `Contents/Resources` second (where `package_app.sh` copies
    /// it), the repo checkout last (dev/test).
    ///
    ...
    /// `bundleResourcesURL` skips its arm. It used to default to the app's
    /// SwiftPM resource bundle (`Bundle.localvoxtralResources`), which the
    /// core can't reach and which holds only `BrandIcons` and `Config`, so
    /// that arm never matched.
    ...
    /// One bundled Vibe file, resolved like `opencodePluginURL`:
    /// `bundleResourcesURL`, the app bundle's `Contents/Resources`, then the
    /// repo checkout (dev/test).
