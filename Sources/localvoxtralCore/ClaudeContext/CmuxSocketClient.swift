...
package struct CmuxSocketClient: CmuxSurfaceQuerying, CmuxSurfaceWriting {
    ...
    ///     is running. Injected for the same reason, and because the core has
    ///     no AppKit: the app passes `runningBundleID(ofPID:)`.
    package init(
    ...
    package static func localPeerPID(_ fd: Int32) -> pid_t? {
    ...
    package static func defaultSocketPaths() -> [String] {
    ...
    package func focusedSurface(expectedPeerPID: pid_t) async -> CmuxQueryResult<CmuxFocusedSurface> {
    ...
    /// becoming our memory.
    package static let maxResponseLineBytes = 1024 * 1024
