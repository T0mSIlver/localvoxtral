...
package enum SSHProxyJumpShape: String, Sendable, Equatable {
    ...
    package init(configuredValue value: String) {
...
package final class SSHDestinationCanonicalizer: Sendable {
    package struct Identity: Sendable, Equatable {
        ...
        package let proxyJump: SSHProxyJumpShape
        ...
        package func matches(_ other: Identity) -> Bool {
            hostname == other.hostname && port == other.port
        }
    }

    package typealias ProcessRunner = @Sendable (
        _ executableURL: URL,
        _ invocation: ClaudeRemoteEnrollmentService.Invocation
    ) throws -> ClaudeRemoteEnrollmentService.RunResult

    package static let executableURL = URL(fileURLWithPath: "/usr/bin/ssh")
    package static let defaultTTL: TimeInterval = 5 * 60
    package static let defaultTimeout: TimeInterval = 2
    ...
    package func proxyJumpShape(for operand: String) async -> SSHProxyJumpShape? {
    ...
    package static func parse(_ output: String) -> Identity? {
        var hostname: String?
        var port: UInt16?
        var user: String?
    ...
    package static func isSSHURIDestination(_ operand: String) -> Bool {
