...
public enum ClaudeShellRCSetup {
    ...
    /// markers.
    static let block = MarkedTextBlock(markerBegin: markerBegin, markerEnd: markerEnd)
    ...
    static func locateBlock(in lines: [String]) -> BlockLocation { block.locateBlock(in: lines) }

    static func splitLines(_ text: String) -> [String] { block.splitLines(text) }

    static func lineTerminator(of text: String) -> String { block.lineTerminator(of: text) }
}
