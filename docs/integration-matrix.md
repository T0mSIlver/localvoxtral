...
| Claude Code, **plain ssh** (no multiplexer) | the TCP connection: the surface's foreground `ssh` process's established socket (local port, peer address, peer port, read from this Mac's kernel) matched to the session's `$SSH_CONNECTION`, pinned to the enrolled host the hook authenticated from | none | no | yes, plus bounded sanitized tool excerpts | remote HTTP | no join through a jump host, inside a multiplexer, or where ControlMaster may be sharing one connection; needs remote plugin ≥ 1.6.0 |
