...
- **Polishing — `localvoxtral-polishd`**, a bundled Swift helper built on
  ...
  Settings). The 9B decodes greedily (temperature 0), so the same
  dictation always polishes the same way; the 4B, the 0.8B, Mistral API
  and External URL polishing send temperature 0.3. A warm prompt cache
  keeps polish latency low, and turning polishing off frees its memory
