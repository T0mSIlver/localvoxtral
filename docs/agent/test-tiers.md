...
| Tier | What | When | Cost |
|---|---|---|---|
| 0 | Unit suite (3,700+ tests) + shell gate suites (run in parallel by `scripts/ci/run-shell-suites.sh`; a new suite is one more argument there, and `test-ui-gate.sh` runs on a PR only when `scripts/ci/ui-gate-suite-filter.sh` says the diff touches a file it reads) + format lint + coverage | every non-fast-path PR/push, in CI's `build-test` job on **GitHub-hosted macOS** (owner decision 2026-09-05 — same-repo PRs too, not just forks; the Mac is the queue bottleneck and hosted runners are free for public repos) | ~4 min hosted, ~0 s queue |
