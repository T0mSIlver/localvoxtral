...
connection cannot identify one window, and says which case it hit: through a
jump host (`ProxyJump`/`ProxyCommand` — the machine you land on sees the jump
host's port, and only the jump host could relate the two), inside
tmux/screen/zellij (a multiplexer keeps the connection that started it), or
when `ControlMaster` is sharing one connection across terminals. In a jumped
or multiplexed session, [herdr](https://herdr.dev) is the arm that works: it
binds the pane rather than the connection.
