...
- Engine, model-pin, prompt, or request-shape changes here MUST run the LLM
  ...
  matches, so run `LV_ALLOW_HEAVY_MAC_RUN=1 ./scripts/remote-build.sh integration-polishd` by hand
...
- Adding a model option to `PolishModelCatalog`: run
  `LV_ALLOW_HEAVY_MAC_RUN=1 ./scripts/remote-build.sh integration-polishd <hf-repo>` as the per-model
